...
Все команды начинаются с `/`. Флаги через `--флаг="значение"`.
...
| `/task --answer="ответ"` | Ответить на вопрос OpenCode (авто-выбор pending-задачи если она одна) |
| `/task --task_key="xxx" --answer="ответ"` | Ответить на вопрос конкретной pending-задачи |
| `/task --parent_task_key="xxx" --parent_match_text="текст"` | Создать follow-up задачу с проверкой вхождения |
| `/task --auto_mode="true"` | Включить авто-режим (бот сам предлагает следующие задачи) |
...
| `/abort` | Прервать единственную работающую задачу (если их несколько — покажет список) |

### Дополнительные флаги `/task`

| Флаг | Описание |
|---|---|
| `--worker="alias"` | Назначить задачу на конкретный worker |
| `--new_session` | Принудительно создать новую сессию OpenCode |
| `--fresh_session` | Аналог `--new_session` |

### Ответы на вопросы OpenCode

Когда OpenCode задаёт вопрос, бот присылает пронумерованные варианты. Ответ:

- **Один вопрос** — просто номер или текст варианта
- **Несколько вопросов** — ответы через `||`: `1 || Prisma`
- **Множественный выбор** — варианты через `&&`: `1 && 3`

Отклонить вопрос: `/reject` или `/task --answer="/reject"`.

### Авто-режим

После включения (`/task --auto_mode="true"`) бот анализирует каждую завершённую задачу и предлагает следующую: продолжение GSD-цикла, проверку качества, тесты, документацию.

Авто-задачи получают префикс `auto-` и наследуют worker от задачи-триггера.
