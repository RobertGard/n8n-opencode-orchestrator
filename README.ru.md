...
Включено в стек автоматически. Первый запуск:

1. `bash ./scripts/setup-stack.sh` запускает HA на порту 8123
2. Откройте HA в браузере, создайте пользователя, Профиль → Длинные токены доступа → создайте токен
3. `bash ./scripts/bootstrap-telegram-integration.sh` запросит токен
4. Установите HA Companion App на телефон, подключитесь к URL HA
