...
- **Верификация приёмки** — передайте `--verify="критерии"` с задачей; после выполнения read-only verifier-агент проверит здоровье кода, логи приложения, консоль браузера (через Playwright), ответы API и каждый критерий. При FAILED авто-создаётся задача-фикс с теми же критериями
