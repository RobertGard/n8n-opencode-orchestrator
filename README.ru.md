...
- **Голосовое управление** — Home Assistant + Companion App. Wake-word «Okay Nabu» → голосовая задача → TTS-ответ. **Полная автонастройка**: Wyoming whisper/piper, voice pipeline, уведомления — ни одного клика в UI. Работает вместе с Telegram
- **Безграничная расширяемость** — весь арсенал n8n (400+ нод) и Home Assistant (тысячи интеграций) в вашем распоряжении: создавайте свои сценарии, подключайте IoT-устройства, синхронизируйте с календарями, базами данных, API — стройте любые автоматизации без ограничений
...
5. Скажите «Okay Nabu» чтобы ставить задачи голосом. Результаты озвучиваются через TTS
