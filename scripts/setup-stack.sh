...
run_startup_pipeline() {
  ...
  if [ -n "${DOCKER_HOST_IP:-}" ] || hostname -I 2>/dev/null | grep -q '[0-9]'; then
    ...
    printf '     → В настройках приложения: Настройки → Голосовой помощник\n'
    printf '       Выбери "Assist" → Настрой → STT/TTS использовать встроенные\n'
    printf '  5. После настройки скажи "Окей, Ассистент" и проверь что HA слышит\n'
    printf '══════════════════════════════════════════\n\n'
  fi
}
